...
const mintTree = async (event, contractInstance) => {
  ...
  // Check if Ethereum is injected into window, and if not, prompt the user to install Metamask
  if (typeof window.ethereum === 'undefined') {
    alert('Looks like you need a Dapp browser to get started.');
    alert('Consider installing MetaMask!');
  } else {
    // User has Metamask installed, request them to connect their wallet
    ethereum.request({ method: 'eth_requestAccounts' }).then(async function(accounts) {
      const to = accounts[0]; // Get the connected user's address
      const tokenId = Date.now(); // Use the current timestamp as a unique tokenId
