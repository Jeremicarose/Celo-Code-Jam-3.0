...
const mnemonic =
  "  pitch enrich theme fish man angry quarter pole size voyage kitchen ghost soul control ski burst during pony blouse";
...
const contractAddress = "0xf572EB2839B769aECa7b317704E7545b2dC01Dae";
const contract = new kit.web3.eth.Contract(contractABI, contractAddress);
const button = document.getElementById('mintTree');
...
async function mintTree() {
  ...
  try {
    ...
    // Get the minted tree info from the contract
    const tokenId = result.events.TreeMinted.returnValues.tokenId;
    const treeInfo = await getTreeInfo(tokenId);
